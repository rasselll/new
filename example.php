...
if ($a == 'cart') {
	...
	echo ' <button class="btn btn-default btn-update-all" data-id="' . $id . '" data-color="' . ((isset($item['attributes']['color'])) ? $item['attributes']['color'] : '') . '"><i class="fa fa-refresh"></i> Update All</button>';
...
<html lang="en">
	...
	<body>
        ...
        $(".thisismyclass").each(function(index,item) {
          ...
          var $btn = $(this);
          var id = $btn.parent().parent().find('.data-id', this).val();
          var qty = $btn.parent().parent().find('.quantity', this).val();
          var color = $btn.parent().parent().find('.data-size', this).val();
          var price = $btn.parent().parent().find('.data-price', this).val();
